...
string knn_grid(double x, double y, char * index_path, int k, int n)
{
	...
	ofstream grid_time;
	grid_time.open("grid_mem_time.txt", ios_base::app);
	grid_time << result << endl;
	grid_time.close();
	...
	*/
	...
	ofstream grid_time_exec;
	grid_time_exec.open(("grid_exec_time" + to_string(n) + ".txt"), ios_base::app);
	grid_time_exec << result << endl;
	grid_time_exec.close();
...
string knn_linear_scan(double x, double y, char * data_path_new, int k)
{
	...
	ofstream linear_time;
	linear_time.open("linear_mem_time.txt", ios_base::app);
	linear_time << result << endl;
	linear_time.close();
	...
	ofstream linear_time_exec;
	linear_time_exec.open("linear_exec_time.txt", ios_base::app);
	linear_time_exec << result << endl;
	linear_time_exec.close();
...
{
	vector<LocationQuery> v = createQuery(100);

	for(int i = 0; i < 100; i++){
		LocationQuery loc = v.back();

		cout << "Testing for query " << i+1 << " Latitude: " << loc.latitude << " Longitude: " << loc.longitude << endl;
		...
		clock_t s, t;
		s=clock();
		cout << "Linear scan results: "<<knn_linear_scan(loc.latitude, loc.longitude, argv[3], atoi(argv[5]))<<endl;
		t=clock();
		cout<<"Linear scan time: "<<(double)(t-s)<<endl;
		
		s=clock();
		cout << "Grid index search results: "<<knn_grid(loc.latitude, loc.longitude, argv[4], atoi(argv[5]), atoi(argv[6]))<<endl;
		t=clock();
